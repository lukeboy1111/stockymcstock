...
public class StockDemo {
	...
	public void doDemo() {
		try {
			for(int i = 0; i < StockConstants.MAX_BUFFER_TRADES; i++) {
				StockMessage outcome = stockService.message(StockConstants.STOCK1, popPrice);
				if(outcome.getSuccess() && outcome.getIsReportable()) {
		...
		try {
			...
			System.out.println("* This is expected:, since trade limit is "+StockConstants.MAX_BUFFER_TRADES+" - message: "+e.getMessage());
		}
		...
		try {
			StockMessage outcome = stockService.message(StockConstants.STOCK1, AdjustmentTypeEnum.ADD, adjustment);
		} catch (StockException e) {
	...
	public void doDemoWithAdjustmentAdd(AdjustmentTypeEnum adjustmentType) {
		...
		try {
			StockMessage outcome = stockService.message(StockConstants.STOCK1, adjustmentType, adjustment);
		} catch (StockException e) {
			System.out.println("Expected: "+e.getMessage());
		}
		report = stockService.message();
